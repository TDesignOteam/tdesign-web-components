...
import { bind, classNames, Component, tag } from 'omi';
...
export class Sidebar extends Component<Props> {
  ...
  onItemClick(htmlItem: SidebarItem) {
    htmlItem.isOpen = !htmlItem.isOpen;
    htmlItem.childrenHeight = htmlItem.children.length * 36;
    this.update();
  }
  ...
  install() {
    ...
    this.props.items.forEach((htmlItem) => {
      htmlItem.childrenHeight = htmlItem.isOpen ? htmlItem.children.length * 36 : 0;
    });
  }
                  ...
                  return (
                    <li>
                      ...
                      >
                        ...
                        >
                          {item.children.map((child: SidebarItem) => this.renderToolTipChild(child))}
                        </ul>
                      ...
                      {hasChildren && this.state.isOpen && (
                        ...
                        >
                          {item.children.map((child: SidebarItem) => this.renderChild(child))}
                        </ul>
                      )}
                    </li>
                  );
