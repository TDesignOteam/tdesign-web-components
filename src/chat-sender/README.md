...
| autosize | Boolean / `{ minRows?: number; maxRows?: number }` | `{ minRows: 2, maxRows: 5 }` | 高度自动撑开，配置项 `{ minRows, maxRows }` | N |
| defaultValue | String / Number | - | 输入框的默认值，非受控模式 | N |
| disabled | Boolean | `false` | 是否禁用输入框 | N |
| fileUploadProps | [TdChatSenderUploadProps](#tdchatsenderuploadprops) | - | 文件上传配置，优先级高于 uploadProps | N |
| footerPrefix | String / Function / TNode | - | 输入框左下角区域扩展 | N |
...
| suffix | String / Function / TNode | - | 输入框右下角区域扩展 | N |
...
| value | String / Number | - | 输入框的值，支持双向绑定（通过 onChange 事件实现） | N |
...
| 名称       | 参数                                                         | 描述                   |
| :--------- | :----------------------------------------------------------- | :--------------------- |
| blur       | `(e: CustomEvent<{ value: string; context: { e: FocusEvent } }>)`         | 输入框失焦时触发       |
| change     | `(e: CustomEvent<{ value: string; context: { e: InputEvent } }>)`         | 输入框值发生变化时触发 |
| fileRemove | `(e: CustomEvent<{ item: [TdAttachmentItem](#tdattachmentitem); index: number }>)` | 移除文件时触发，参数为被删除的文件项和索引 |
| fileSelect | `(e: CustomEvent<[TdAttachmentItem](#tdattachmentitem)[]>)` | 选择文件时触发，返回新选择的文件列表 |
| focus      | `(e: CustomEvent<{ value: string; context: { e: FocusEvent } }>)`         | 输入框聚焦时触发       |
| send       | `(e: CustomEvent<[TdChatSenderParams](#tdchatsenderparams)>)`  | 点击消息发送时触发     |
| stop       | `(e: CustomEvent<{ value: string; context: { e: MouseEvent } }>)`         | 点击消息终止时触发     |

### ChatSender Slots

| 名称          | 说明                 |
| :------------ | :------------------- |
| actions       | 输入框右下角区域扩展，也可通过 `suffix` 属性设置 |
| footer-prefix | 输入框左下角区域扩展，也可通过 `footerPrefix` 属性设置 |
| header        | 输入框外标题区域扩展 |
...
| :---------- | :-------------------------------------------- | :--------- |
| value       | String                                        | 输入框值   |
| context     | `{ e?: MouseEvent \| KeyboardEvent }`         | 上下文对象 |
