...
export const adoptStyleSheet = (root: Document | ShadowRoot, style: string) => {
  try {
    const preStyleSheet = root.adoptedStyleSheets?.find(
      (item) => (item as CSSStyleSheet & { styleStr?: string }).styleStr === style,
    );
    if (preStyleSheet) return;

    const styleSheet = createStyleSheet(style);
    root.adoptedStyleSheets = root.adoptedStyleSheets ? root.adoptedStyleSheets.concat(styleSheet) : [styleSheet];
  } catch {
