...
| 名称             | 类型                                                               | 默认值                                       | 说明                                               | 必传 |
| ---------------- | ------------------------------------------------------------------ | -------------------------------------------- | -------------------------------------------------- | ---- |
| actions          | `Array<TdChatMessageActionName \| TdChatMessageAction> \| boolean` | `['replay', 'copy', 'good', 'bad', 'share']` | 消息操作按钮及顺序；可用 `{ name, render }` 自定义 | N    |
| animation        | `ChatLoadingAnimationType`                                         | `'skeleton'`                                 | 消息加载动画                                       | N    |
| handleActions    | `Partial<Record<TdChatMessageActionName, (data?: any) => void>>`   | -                                            | 操作按钮回调                                       | N    |
...
`TdChatMessageActionName` 包含 `copy`、`good`、`bad`、`replay`、`share`、`searchResult`、`searchItem`、`suggestion` 和 `codeCopy`。
