...
export const navbarItems = signal<NavbarItem[]>([
  ...
  {
    href: 'https://github.com/TDesignOteam/tdesign-web-components',
    target: '_blank',
...
export function getValueByPath(path: string, navbarItems: NavbarItem[]): string | null {
  for (let i = 0; i < navbarItems.length; i++) {
    ...
    if (navbarItems[i].children) {
      // @ts-ignore
      const found = getValueByPath(path, navbarItems[i].children);
...
export const sidebarItems = signal<NavbarItem[]>(sidebarConfig);

export const activeSidebarItem = signal('button');
